...
class QuestsScreen : AbstractScreen()
{
	...
	fun createHubTable()
	{
		...
		bountyLabelTable.add(Label("Bounties", Statics.skin))
		...
		expeditionTable.row()
		expeditionTable.add(Label("Expedition", Statics.skin)).padBottom(16f)
	...
	fun createBountiesTable()
	{
		...
		mainTable.add(gameDataBar).growX()
		mainTable.row()
		mainTable.add(Label("Bounties", Statics.skin, "title")).expandX().center().padTop(20f)
		mainTable.row()
		mainTable.add(Seperator(Statics.skin, false)).growX().pad(5f)
		mainTable.row()
		...
		for (bounty in bounties)
		{
			...
			fun fillTable()
			{
				...
				if (bounty == null)
				{
					...
					bountyTable.add(Label("Tap to select a bounty", Statics.skin)).expandX()
					bountyTable.add(Label("-- : -- : --", Statics.skin))
				...
				else
				{
					...
					bountyTable.add(Label(bounty!!.title, Statics.skin).wrap().tint(bounty!!.ascension.colour.copy().lerp(Colour.WHITE, 0.5f).color())).growX().padLeft(10f).padRight(5f)

					if (bounty!!.isComplete())
					{
						val takeButton = TextButton("Collect", Statics.skin)
						...
						val widthLabel = Label("-- : -- : --", Statics.skin)
		...
		nextBountyTable.add(Label("Next bounty slot at " + Global.data.nextBountyLevel(), Statics.skin))
